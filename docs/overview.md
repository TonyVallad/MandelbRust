...
2. **Selective supersampling** — boundary pixels are re-sampled at sub-pixel positions (2×2 or 4×4 grid; choose AA level via `A` key (cycles) or the deblur toolbar icon popup (Off / 2×2 / 4×4)). Interior pixels are untouched.
...
- Palette editor and advanced coloring are available via the **Display/color settings** panel (opened from the toolbar). It supports palette mode (cycles vs cycle length), start-from black/white, smooth/log-log toggle, and **color profiles** (one file per profile in a `color_profiles/` folder). It also includes coloring mode selection (Standard, Histogram, Distance Estimation) and interior coloring (Black, Stripe Average).
...
| **Display/color settings (tabbed)** | **Icon:** Toolbar display/color button opens a tabbed panel (Profiles / Palette / Coloring / Interior). **Palette editor:** gradient bar with draggable stops; create/rename/delete user palettes; start/end fades. **Coloring modes:** Standard, Histogram, Distance Estimation; **Interior:** Black and Stripe Average. **Color profiles:** one file per profile in a `color_profiles/` folder; **Bookmarks:** save and restore a full display/color snapshot (including coloring + interior choices). |
...
- GPU compute backend
- Advanced coloring (palette editor, Histogram/Distance Estimation, interior stripe average) — implemented in v0.1.0-alpha.
